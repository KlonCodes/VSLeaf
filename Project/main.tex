...
%% Rights management -- sent when you complete the rights form.
...
%% Submission ID. -  when submitting an article to a sponsored event.
%%\acmSubmissionID{123-A56-BU3}

%% If for an event sponsored by ACM SIGGRAPH, use "author year" style.
...
\begin{teaserfigure}
  \centering
  \includegraphics[width=0.8\textwidth]{../../.vscode/VSleaf.png}
...
\subsection{Nested}
\par Citations: \cite{knuth1990}.
\subsubsection{Nested further}
\par Sub-sub goes here, same line.
...
\par But, we could also do a \chl[klon]{use chl to add a hover comment with highlight text link} less-flow-breaking comment.
\begin{comment}
  We can do code comments in here for just us comments like this.
% doing this is also okay, for shorter comments.
\end{comment}
